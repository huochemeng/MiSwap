...
func GetMultiChainUserCollections(ctx context.Context, svcCtx *svc.ServerCtx, chainIDs []int, chainNames []string, userAddrs []string) (*dto.UserCollectionsResp, error) {
	...
	// 6. 组装最终结果
	var results dto.UserCollectionsData
	chainInfos := make(map[int]dto.ChainInfo)
	for _, collection := range collections {
		// 6.1 添加Collection信息
		listCount := collectionsListed[strings.ToLower(collection.Address)]
		results.CollectionInfos = append(results.CollectionInfos, dto.CollectionInfo{
		...
		if ok {
			...
			chainInfos[collection.ChainID] = dto.ChainInfo{
				ChainID:   collection.ChainID,
	...
	return &dto.UserCollectionsResp{
		Result: results,
	}, nil
}

// GetMultiChainUserItems 查询用户拥有nft的Item基本信息，list信息和bid信息，从Item表和Activity表中查询
func GetMultiChainUserItems(ctx context.Context, svcCtx *svc.ServerCtx, chainID []int, chain []string, userAddrs []string, contractAddrs []string, page, pageSize int) (*dto.UserItemsResp, error) {
	// 1.
	items, count, err := svcCtx.Dao.QueryMultiChainUserItemInfos(ctx, chain, userAddrs, contractAddrs, page, pageSize)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get user items info")
	}

	// 如果没有Item,直接返回空结果
	if count == 0 {
		return &dto.UserItemsResp{
			Result: items,
			Count:  count,
		}, nil
	}
	...
	var multichainItems = make(map[string][]dto.ItemInfo) // 按链分组的Item信息
	...
	for _, item := range items {
		...
		multichainItems[chainIDToChainName[item.ChainID]] = append(multichainItems[chainIDToChainName[item.ChainID]], dto.ItemInfo{
			CollectionAddress: item.CollectionAddress,
			TokenID:           item.TokenID,
		})
	}
	...
	collectionBestBids := make(map[dto.MultichainCollection]model.Order)
	var wg sync.WaitGroup
	var mu sync.Mutex
	var queryErr error
	for chain, collections := range chainCollections {
		...
		go func(chainName string, collectionArray []string) {
			...
			if err != nil {
				queryErr = errors.Wrap(err, "failed to query collections best bids")
				return
			}
			mu.Lock()
			defer mu.Unlock()
			for _, bestBid := range bestBids {
				collectionBestBids[dto.MultichainCollection{
	...
	wg.Wait()
	if queryErr != nil {
		return nil, errors.Wrap(err, "failed to query collection bids")
	}

	// 6. 并发查询Item最高出价信息
	itemsBestBids := make(map[dao.MultiChainItemInfo]model.Order)
	for chain, items := range multichainItems {
		wg.Add(1)
		go func(chainName string, itemInfos []dto.ItemInfo) {
			defer wg.Done()
			bids, err := svcCtx.Dao.QueryItemsBestBids(ctx, chainName, userAddr, itemInfos)
			if err != nil {
				queryErr = errors.Wrap(err, "failed to query items best bids")
				return
			}
			...
			for _, bid := range bids {
				order, ok := itemsBestBids[dao.MultiChainItemInfo{ItemInfo: dto.ItemInfo{CollectionAddress: strings.ToLower(bid.CollectionAddress), TokenID: bid.TokenId}, ChainName: chainName}]
				if !ok {
					itemsBestBids[dao.MultiChainItemInfo{ItemInfo: dto.ItemInfo{CollectionAddress: strings.ToLower(bid.CollectionAddress), TokenID: bid.TokenId}, ChainName: chainName}] = bid
					continue
				}
	...
	wg.Wait()
	if queryErr != nil {
		return nil, errors.Wrap(err, "failed to query items best bids")
	}

	// 7. 查询Collection信息
	collections, err := svcCtx.Dao.QueryMultiChainCollectionsInfo(ctx, collectionAddrs)
	if err != nil {
		return nil, errors.Wrap(err, "failed to query collections info")
	}

	collectionInfos := make(map[string]model.Collection)
	for _, collection := range collections {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to query item list info")
	}
	...
	for _, item := range listingInfos {
		if item.Listing {
			itemPrice = append(itemPrice, dao.MultiChainItemPriceInfo{
				ItemPriceInfo: dto.ItemPriceInfo{
					CollectionAddress: item.CollectionAddress,
					TokenID:           item.TokenId,
					Maker:             item.Owner,
	...
	// 10. 查询挂单订单信息
	orderIds := make(map[string]model.Order)
	if len(itemPrice) > 0 {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to query item image info")
	}
	...
	for i := 0; i < len(items); i++ {
		// 设置出价信息
		bidOrder, ok := itemsBestBids[dao.MultiChainItemInfo{ItemInfo: dto.ItemInfo{CollectionAddress: strings.ToLower(items[i].CollectionAddress), TokenID: items[i].TokenID}, ChainName: chainIDToChainName[items[i].ChainID]}]
		if ok {
			if bidOrder.Price.GreaterThan(collectionBestBids[dto.MultichainCollection{
			...
			if cBid, ok := collectionBestBids[dto.MultichainCollection{
				CollectionAddress: strings.ToLower(items[i].CollectionAddress),
				Chain:             chainIDToChainName[items[i].ChainID],
			}]; ok {
...
func GetMultiChainUserListings(ctx context.Context, svcCtx *svc.ServerCtx, chainID []int, chain []string, userAddrs []string, contractAddrs []string, page, pageSize int) (*dto.UserListingsResp, error) {
	...
	for i := 0; i < len(items); i++ {
		...
		if ok {
			...
			bidOrder, ok := collectionBestBids[dto.MultichainCollection{
				CollectionAddress: strings.ToLower(items[i].CollectionAddress),
				Chain:             chainIDToChainName[items[i].ChainID],
			}]
...
func GetMultiChainUserBids(ctx context.Context, svcCtx *svc.ServerCtx, chainID []int, chainNames []string, userAddrs []string, contractAddrs []string, page, pageSize int) (*dto.UserBidsResp, error) {
	// 1. 遍历每条链,查询用户出价信息
	...
	for i, chain := range chainNames {
		...
		if err != nil {
			return nil, errors.Wrap(err, "failed to get user bids info")
		}
	...
	bidsMap := make(map[string]dto.UserBid)
	...
	for _, bid := range totalBids {
		...
		userBid.BidInfos = append(userBid.BidInfos, dto.BidInfo{
			BidOrderID:    bid.OrderID,
			BidTime:       bid.EventTime,
			BidExpireTime: bid.ExpireTime,
			BidPrice:      bid.Price,
	...
	for chain, collections := range bidCollections {
		...
		if err != nil {
			return nil, errors.Wrap(err, "failed to get collections info")
		}
