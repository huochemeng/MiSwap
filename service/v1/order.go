...
func GetOrderInfos(ctx context.Context, svcCtx *svc.ServerCtx, chainID int, chain string, userAddr string, collectionAddr string, tokenIds []string) ([]dto.ItemBid, error) {
	// 1. 构建NFT信息列表
	...
	itemsBestBids := make(map[string]model.Order)
...
//
// 3. 返回每个NFT的最终出价信息
func processBids(tokenIds []string, itemsBestBids map[string]model.Order, collectionBids []model.Order, collectionAddr string) []dto.ItemBid {
	// 将itemsBestBids转换为切片并按价格升序排序
	var itemsSortedBids []model.Order
	...
	for _, tokenId := range tokenIds {
		if _, ok := itemsBestBids[tokenId]; !ok {
			...
			if cBidIndex < len(collectionBids) {
				resultBids = append(resultBids, dto.ItemBid{
