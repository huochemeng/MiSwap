...
const CacheActivityNumPrefix = "cache:activity:count:"

var eventTypesToID = map[string]int{
	"sale":                  model.Sale,
	"transfer":              model.Transfer,
	"offer":                 model.MakeOffer,
	"cancel_offer":          model.CancelOffer,
	"cancel_list":           model.CancelListing,
	"list":                  model.Listing,
	"mint":                  model.Mint,
	"buy":                   model.Buy,
	"collection_bid":        model.CollectionBid,
	"item_bid":              model.ItemBid,
	"cancel_collection_bid": model.CancelCollectionBid,
	"cancel_item_bid":       model.CancelItemBid,
}
...
var idToEventTypes = map[int]string{
	...
	model.MakeOffer:           "offer",
	model.CancelOffer:         "cancel_offer",
	model.CancelListing:       "cancel_list",
	model.Listing:             "list",
	model.Mint:                "mint",
	model.Buy:                 "buy",
	model.CollectionBid:       "collection_bid",
	model.ItemBid:             "item_bid",
	model.CancelCollectionBid: "cancel_collection_bid",
	model.CancelItemBid:       "cancel_item_bid",
}
...
// - int64: 总记录数
// - error: 错误信息
func (d *Dao) QueryMultiChainActivities(ctx context.Context, chainName []string, collectionAddrs []string, tokenID string,
	userAddrs []string, eventTypes []string, page, pageSize int) ([]ActivityMultiChainInfo, int64, error) {
	//查询缓存中的总数
	var strNums []string
	...
	for _, chain := range chainName {
		...
		sqlMid += fmt.Sprintf("from %s ", model.ActivityTableName(chain))
...
// 包括: 用户地址、NFT信息、合约信息等
func (d *Dao) QueryMultiChainActivityExternalInfo(ctx context.Context, chainID []int, chainName []string, activities []ActivityMultiChainInfo) ([]dto.ActivityInfo, error) {
	// 收集需要查询的地址和ID
	...
	itemInfos := make(map[string]model.Item)
	...
	go func() {
		...
		for i := 0; i < len(itemQuery); i++ {
			...
			itemDb := d.DB.WithContext(ctx).
				Table(model.ItemTableName(items[i][2])).
	...
	go func() {
		defer wg.Done()
		var newItems []model.ItemExternal
		var newItem model.ItemExternal
		...
		for i := 0; i < len(itemQuery); i++ {
			...
			// WHERE (collection_address, token_id) = (?, ?)
			itemDb := d.DB.WithContext(ctx).
				Table(model.ItemExternalTableName(items[i][2])).
	...
	go func() {
		...
		for i := 0; i < len(collectionAddrs); i++ {
			...
			if err := d.DB.WithContext(ctx).
				Table(model.CollectionTableName(collectionAddrs[i][1])).
	...
	for _, act := range activities {
		...
		if act.ActivityType == model.Listing {
			activity.TxHash = ""
		}
