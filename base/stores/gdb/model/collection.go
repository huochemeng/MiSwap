...
type Collection struct {
	...
	FloorPrice       decimal.Decimal `gorm:"column:floor_price" json:"floor_price"`                      //地板价，最低挂牌价。代表卖家的心理预期，是当前市场上所有未成交挂单中的最低价
	SalePrice        decimal.Decimal `gorm:"column:sale_price" json:"sale_price"`                        //成交价，实际卖出价。代表真实的交易结果
...
// TableName gorm固定的函数，不能传入参数
//func (c Collection) TableName() string {
//	return "collection"
//}

func CollectionTableName(chainName string) string {
	return fmt.Sprintf("collection_%s", chainName)
}
